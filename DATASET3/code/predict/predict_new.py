...
# ------------------- 数据准备 -------------------
# 读取温度分布数据
df = pd.read_csv(r"E:\MASTER\2-spring\algae\5_8\processed_data\histogram_all_except.csv")
...
def prepare_data(df):
    ...
    pivot_df = df.pivot_table(
        index=["time", "condition"],  # 只使用time作为索引
        columns="area_bin",
        values="frequency",
        aggfunc="first"
    )
    ...
    X = pivot_df.values
    # 读取growth_rate.csv
    df_mu = pd.read_csv(r"E:\MASTER\2-spring\algae\5_8\processed_data\growth_rate_all_except.csv")
    # 直接获取mu列的值
    y = df_mu["mu"].values
