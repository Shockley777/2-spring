"""
多模型基准评测 - DATASET2 (C/N比率条件)

功能：
- 基于 DATASET2 的面积直方图分布数据进行建模
- 特征：面积分布直方图 + 时间 + 条件值 + 统计特征
- 统一随机划分训练/测试（总样本36，训练24，测试12）
...
def load_dataset() -> Tuple[pd.DataFrame, pd.DataFrame]:
    """加载DATASET2的直方图数据和生长率数据"""
    # 可能的路径候选
    hist_candidates = [
        r"ds2\processed_data\histogram_data.csv",
        r"ds2/processed_data/histogram_data.csv",
        os.path.join("ds2", "processed_data", "histogram_data.csv"),
        os.path.join("DATASET2", "processed_data", "histogram_data.csv"),
    ]
    
    mu_candidates = [
        r"ds2\processed_data\growth_rate.csv",
        r"ds2/processed_data/growth_rate.csv",
        os.path.join("ds2", "processed_data", "growth_rate.csv"),
        os.path.join("DATASET2", "processed_data", "growth_rate.csv"),
    ]
...
def plot_results(df_results: pd.DataFrame, save_dir: str) -> None:
    ...
    plt.title("DATASET2 模型对比 (Test RMSE)")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "dataset2_model_comparison_test_rmse.png"), dpi=200)
    plt.close()
    ...
    plt.xticks(rotation=35, ha="right")
    plt.title("DATASET2 模型对比 (5-fold CV RMSE)")
    plt.ylabel("RMSE")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "dataset2_model_comparison_cv_rmse.png"), dpi=200)
    plt.close()
    ...
    plt.xticks(rotation=35, ha="right")
    plt.title("DATASET2 模型对比 (Test R2)")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "dataset2_model_comparison_test_r2.png"), dpi=200)
    plt.close()


def main() -> None:
    print("[信息] 加载 DATASET2 数据...")
    try:
        df_hist, df_mu = load_dataset()
        df = pd.merge(df_hist, df_mu, on=["time", "condition_value"])
        print(f"[信息] 数据加载成功，样本数量: {len(df.groupby(['time', 'condition_value']))}")
    except Exception as e:
    ...
    print("\n=== DATASET2 Benchmark Results (sorted by Test RMSE) ===")
    print(pretty.to_string(index=False, float_format=lambda v: f"{v:.4f}"))

    # 保存结果
    save_dir = os.path.join("DATASET2", "results")
    os.makedirs(save_dir, exist_ok=True)
    csv_path = os.path.join(save_dir, "dataset2_model_benchmark_results.csv")
