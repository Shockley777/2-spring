...
def visualize_predictions(loader):
    ...
    for i, (true, pred) in enumerate(zip(targets, preds)):
        ...
        print(f"Sample {i+1}: True = {true:.4f}, Predicted = {pred:.4f}, Error = {abs_error:.4f}, Relative Error = {rel_error:.2f}%")
...
def train_and_evaluate():
    ...
    confidence_interval = 1.96 * (std_rel_error / np.sqrt(n))  # 95% 置信区间
    
    print("\nRelative Error Statistics (%):")
    print(f"Mean Relative Error: {mean_rel_error:.2f}%")
    print(f"95% Confidence Interval: [{mean_rel_error - confidence_interval:.2f}%, {mean_rel_error + confidence_interval:.2f}%]")
    print(f"Median Relative Error: {np.median(relative_errors):.2f}%")
    print(f"Relative Error Standard Deviation: {std_rel_error:.2f}%")
    print(f"Maximum Relative Error: {np.max(relative_errors):.2f}%")
    print(f"Minimum Relative Error: {np.min(relative_errors):.2f}%")
    
    return model, test_pred, relative_errors

# ------------------- 交叉验证 -------------------
def cross_validate():
    kf = KFold(n_splits=5, shuffle=True, random_state=SEED)
    cv_scores = []
    cv_rel_errors = []  # 存储所有折的相对误差
    ...
    print(f"\nCross-validation Relative Error Statistics (%):")
    print(f"Mean Relative Error: {mean_cv_rel_error:.2f}%")
    print(f"95% Confidence Interval: [{mean_cv_rel_error - cv_confidence_interval:.2f}%, {mean_cv_rel_error + cv_confidence_interval:.2f}%]")
    print(f"Median Relative Error: {np.median(cv_rel_errors):.2f}%")
    print(f"Relative Error Standard Deviation: {std_cv_rel_error:.2f}%")
    print(f"Maximum Relative Error: {np.max(cv_rel_errors):.2f}%")
    print(f"Minimum Relative Error: {np.min(cv_rel_errors):.2f}%")

# ------------------- 执行流程 -------------------
if __name__ == "__main__":
    print("Training model...")
    model, predictions, rel_errors = train_and_evaluate()
