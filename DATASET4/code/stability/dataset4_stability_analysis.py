...
# -------- 主分析函数 --------
def analyze_dataset4_stability():
    print("🔍 开始分析DATASET4的细胞面积分布稳定性...")
...
def save_detailed_results(results, recommended_size):
    ...
    output_file = os.path.join(script_dir, "dataset4_stability_analysis.xlsx")
...
def plot_summary_results(results, stable_sizes, recommended_size):
    ...
    plt.tight_layout()
    script_dir = os.path.dirname(os.path.abspath(__file__))
    image_file = os.path.join(script_dir, "dataset4_stability_summary.png")
...
def plot_all_similarity_curves(results, recommended_size):
    ...
    # 保存图片到当前脚本所在目录
    script_dir = os.path.dirname(os.path.abspath(__file__))
    image_file = os.path.join(script_dir, "dataset4_all_similarity_curves.png")
    plt.savefig(image_file, dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    print("🚀 开始DATASET4稳定性分析...")
    print("=" * 60)
    
    result_data = analyze_dataset4_stability()
    ...
    print("\n" + "=" * 60)
    print("✅ 分析完成！")
    print(f"🎯 推荐用于整个DATASET4的代表性样本数量: {recommended_size}")
    
    # 显示输出文件的完整路径
    script_dir = os.path.dirname(os.path.abspath(__file__))
    excel_file = os.path.join(script_dir, "dataset4_stability_analysis.xlsx")
    summary_image_file = os.path.join(script_dir, "dataset4_stability_summary.png")
    curves_image_file = os.path.join(script_dir, "dataset4_all_similarity_curves.png")
