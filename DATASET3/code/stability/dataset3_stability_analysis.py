...
def get_similarity_at_sample_size(result, target_sample_size):
    ...
    # 找到最接近目标样本数的索引
    if target_sample_size in sample_sizes:
        index = sample_sizes.index(target_sample_size)
        return similarities[index]
    
    # 如果目标样本数不在列表中，进行插值
    for i, size in enumerate(sample_sizes):
        if size > target_sample_size:
            if i == 0:
            ...
            ratio = (target_sample_size - prev_size) / (size - prev_size)
            return prev_sim + ratio * (curr_sim - prev_sim)
...
def analyze_single_file(csv_path, area_col=AREA_COL):
    ...
    try:
        ...
        # 计算有效范围内的细胞数（仅用于统计）
        valid_cells = len(area_data[(area_data >= RANGE[0]) & (area_data <= RANGE[1])])
        ...
        return {
            ...
            'valid_cells': valid_cells,      # 有效范围内细胞数
...
# -------- 主分析函数 --------
def analyze_dataset3_stability():
    """分析整个DATASET3的稳定性"""
    print("🔍 开始分析DATASET3的细胞面积分布稳定性...")
...
def save_detailed_results(results, recommended_size):
    ...
    for r in results:
        # 计算在推荐样本数时的相似度
        similarity_at_recommended = get_similarity_at_sample_size(r, recommended_size) if r['stable_sample_size'] != -1 else None
        
        data.append({
            'file_path': r['file_path'],
            'total_cells': r['total_cells'],
            'valid_cells_500_3500': r['valid_cells'],  # 新增列：有效范围内细胞数
            'stable_sample_size': r['stable_sample_size'] if r['stable_sample_size'] != -1 else None,
            f'similarity_at_{recommended_size}': similarity_at_recommended,  # 在推荐样本数时的相似度
            'above_recommended': r['stable_sample_size'] > recommended_size if r['stable_sample_size'] != -1 else False,
    ...
    output_file = os.path.join(script_dir, "dataset3_stability_analysis.xlsx")
...
def plot_summary_results(results, stable_sizes, recommended_size):
    ...
    # 保存图片到当前脚本所在目录
    script_dir = os.path.dirname(os.path.abspath(__file__))
    image_file = os.path.join(script_dir, "dataset3_stability_summary.png")
...
def plot_all_similarity_curves(results, recommended_size):
    ...
    # 保存图片到当前脚本所在目录
    script_dir = os.path.dirname(os.path.abspath(__file__))
    image_file = os.path.join(script_dir, "dataset3_all_similarity_curves.png")
    plt.savefig(image_file, dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    print("🚀 开始DATASET3稳定性分析...")
    print("=" * 60)
    
    result_data = analyze_dataset3_stability()
    ...
    print("\n" + "=" * 60)
    print("✅ 分析完成！")
    print(f"🎯 推荐用于整个DATASET3的代表性样本数量: {recommended_size}")
    
    # 显示输出文件的完整路径
    script_dir = os.path.dirname(os.path.abspath(__file__))
    excel_file = os.path.join(script_dir, "dataset3_stability_analysis.xlsx")
    summary_image_file = os.path.join(script_dir, "dataset3_stability_summary.png")
    curves_image_file = os.path.join(script_dir, "dataset3_all_similarity_curves.png")
